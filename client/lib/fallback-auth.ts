...
export const fallbackSignIn = async (
  email: string,
  password: string,
): Promise<FallbackUserCredential> => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      ...
      if (process.env.NODE_ENV === "development") {
        console.log(
          "🔧 Development mode - trying multiple matching strategies",
        );

        // Strategy 1: Exact match
        let user = FALLBACK_USERS.find(
          ...
          if (user.userType === "merchant") {
            ...
            try {
              const storedApplications =
                localStorage.getItem("storeApplications");
              if (storedApplications) {
                ...
                if (merchantApp) {
                  ...
                  console.log(
                    "🏪 Found merchant data from application:",
                    actualFirstName,
                    actualLastName,
                  );
                }
              ...
              console.error(
                "Error loading merchant data from applications:",
                error,
              );
            }

            console.log(
              "🏪 Generated/Retrieved merchant ID for",
              email,
              ":",
              uniqueUid,
            );
...
export const fallbackCreateUser = async (
  email: string,
  password: string,
  userType?: string,
): Promise<FallbackUserCredential> => {
