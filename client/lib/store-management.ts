  ...
  window.addEventListener("message", (e) => {
    if (e.data.type === "DATA_UPDATE") {
      ...
      window.dispatchEvent(
        new CustomEvent(`${e.data.dataType}Updated`, {
          detail: e.data.data,
        }),
      );
    }
...
export const getCategories = (storeId?: string): Category[] => {
  ...
  return storeId
    ? categories.filter((category) => category.storeId === storeId)
    : categories;
};
...
// في store-management.ts
export const createInitialStoreEnhanced = async (
  uid?: string,
  userData?: Omit<UserData, "uid" | "createdAt" | "updatedAt" | "isActive">,
) => {
  // منطق إنشاء المتجر الافتراضي
  const initialStore = {
  ...
  // إذا كنت تستخدم Firestore، يمكنك حفظه
  /*
  await setDoc(doc(db, 'stores', initialStore.storeId), initialStore);
  */
