...
export const complianceSystem = {
  ...
  // 🔥 7. اكتشاف النوع التفصيلي بناءً على kind والبيانات
  ...
  // 🔥 8. تحضير بيانات المنتج للحفظ حسب النوع
  // 🔥 8. تحضير بيانات المنتج للحفظ حسب النوع
  prepareProductDataForSaving(
    ...
    // 🔥 **أضف هذا القسم: معالجة الحقول الزراعية**
    // التحقق من وجود أي من الحقول الزراعية
    // كان: rawData.isOrganic
    // أصبح: rawData.isOrganic !== undefined
    ...
    try {
      ...
      if (productType && store) {
        ...
        if (productActivity && !storeActivities.includes(productActivity)) {
          ...
          // ⚠️ إزالة هذا السطر: لا نغير القرار
  ...
  // 🔹 10. التحقق من التوافق مع المتجر
  async checkComplianceWithStore(
    ...
    // 1. التحقق من تطابق النشاط
    const storeActivities = store.businessActivities?.subActivities || [];
    const productActivity = productType.activityId;

    if (productActivity && !storeActivities.includes(productActivity)) {
      violations.push(`نشاط المنتج (${productActivity}) غير مسجل للمتجر`);
      suggestedActions.push("يمكنك طلب إضافة النشاط من إعدادات المتجر");

      // إضافة shadow actions للنشاط غير المطابق
      shadowActions = {
        hideFromSearch: true,
        limitPurchase: true,
      };
...
export class ProductService {
  ...
  private convertFirestoreDataToProduct(
    ...
    const convertSemantics = (semanticsData: any): Product["_semantics"] => {
      ...
      // ⭐ ⭐ ⭐ **التحديث المهم: تصحيح حالة الامتثال للمتاجر الزراعية القديمة**
      if (store && semantics.detectedActivity === "agriculture") {
        // الحصول على أنشطة المتجر من مصادر مختلفة
        ...
        // 2. من primaryBusinessType القديم (إذا كان موجودًا في customization)
        else if (
          store.customization &&
          "primaryBusinessType" in store.customization
        ) {
          const oldTypeToActivity: Record<string, string> = {
          ...
          const activity = oldTypeToActivity[oldType];
          if (activity) {
            storeActivities = [activity];
          }
        }
        // 3. من subBusinessTypes القديم (إذا كان موجودًا في customization)
        if (store.customization && "subBusinessTypes" in store.customization) {
          const subTypes = (store.customization as any).subBusinessTypes || [];
          const typeToActivity: Record<string, string> = {
            "agricultural-products": "agriculture",
            livestock: "livestock",
            "agricultural-tools": "agriculture",
          ...
          for (const subType of subTypes) {
            const activity = typeToActivity[subType];
            if (activity && !storeActivities.includes(activity)) {
              storeActivities.push(activity);
            }
          }
        }

        // ⭐ تصحيح حالة الامتثال إذا كان المنتج زراعي والمتجر زراعي
        if (storeActivities.includes("agriculture")) {
  ...
  private async updateStoreComplianceStats(
    ...
    try {
      ...
      // إذا كان تحديث (وليس إضافة)، نعدل الأرقام
  ...
  /**
   * إنشاء منتج جديد بالنظام الجديد
   */
  // 🔥 تحديث دالة create لدعم kind
  // 🔥 تحديث دالة create لدعم kind مع إصلاح الأخطاء
  async create(
    productData: any,
    options?: {
    ...
    try {
      ...
      // 🔍 التحقق الفوري من البيانات المخزنة
      try {
        ...
        // 🔧 إذا كانت metadata مفقودة في البيانات المخزنة، حاول إضافتها يدوياً
  ...
  // 🔥 دالة مساعدة: الحصول على النوع الافتراضي لل kind
  private getDefaultProductTypeForKind(
    ...
    // 🔥 أولوية قصوى: إذا كان المتجر زراعي والنوع مادي
    ...
    // 🔥 تحسين: البحث عن أنواع متوافقة مع النشاط الزراعي
    ...
    // البحث عن نوع منتج يتوافق مع kind
    ...
    // إذا لم يوجد، نعيد النوع الأول المناسب
    switch (kind) {
  ...
  async update(
    ...
    try {
      ...
      // 🔹 10. تحديث الإحصائيات إذا تغيرت الحالة
  ...
  /**
   * الحصول على منتجات المتجر (مبسط - بدون ترقيم متقدم)
   */
  /**
   * الحصول على منتجات المتجر (مبسط - بدون ترقيم متقدم)
   */
  async getByStore(
    ...
    try {
      ...
      // ⭐ ⭐ ⭐ **إضافة: استعلام منفصل لفحص جميع المنتجات**
      ...
      // ⭐ إضافة type assertion لبيانات Firestore
      ...
      // ⭐ ⭐ ⭐ **إصلاح: إنشاء الاستعلام مع أو بدون orderBy**
      let q;
      if (status === "all") {
      ...
      for (const doc of querySnapshot.docs) {
        try {
          ...
          // ⭐ ⭐ ⭐ **تمرير بيانات المتجر إلى convertFirestoreDataToProduct**
          ...
          // ⭐ سجل خاص للمنتجات الزراعية للتشخيص
          if (product._semantics?.detectedActivity === "agriculture") {
            console.log(`🌱 منتج زراعي: ${product.name}`, {
              ...
              storeHasAgriculture:
                store?.businessActivities?.mainActivity === "agriculture" ||
                store?.businessActivities?.subActivities?.some((type: string) =>
                  [
  ...
  /**
   * الحصول على المنتجات ذات أعلى تخفيض
   */
  ...
  /**
   * الحصول على اقتراحات لأنواع المنتجات
   */
  ...
  /**
   * إصلاح المنتجات الزراعية للمتاجر الزراعية القديمة
   */
  ...
  /**
   * دالة مساعدة: التحقق إذا كان المتجر زراعي
   */
  private isStoreAgricultural(store: Store): boolean {
    // التحقق من businessActivities الجديد
    if (store.businessActivities?.subActivities?.includes("agriculture")) {
      return true;
    }

    // التحقق من customization القديم
    if (store.customization) {
      // primaryBusinessType القديم
      if (
        "primaryBusinessType" in store.customization &&
        (store.customization as any).primaryBusinessType === "agriculture"
      ) {
        return true;
      }

      // subBusinessTypes القديم
      if ("subBusinessTypes" in store.customization) {
        ...
        if (
          subTypes.some((type: string) => agricultureSubTypes.includes(type))
        ) {
          return true;
        }
      }
...
export const orderService = {
  ...
  async getByCustomerAndStore(
    ...
    try {
      const q = query(
        ...
        where("storeId", "==", storeId),
        // ⚠️ ملاحظة: تجنب إضافة orderBy هنا إذا لم يكن لديك Index مركب
      );
      ...
      // الترتيب على مستوى التطبيق لتجنب مشاكل الفهرسة
      orders.sort((a, b) => {
        const aDate =
          a.createdAt instanceof Date ? a.createdAt : new Date(a.createdAt);
        const bDate =
          b.createdAt instanceof Date ? b.createdAt : new Date(b.createdAt);
        return bDate.getTime() - aDate.getTime(); // الأحدث أولاً
      });

      return orders;
    } catch (error) {
      console.error("Error getting orders by customer and store:", error);
      return [];
    }
  },
  // Get orders by store
  ...
  // Merchant marks an order as delivered (marks that merchant confirmed delivery)
  async merchantMarkAsDelivered(orderId: string): Promise<void> {
    const cleanedData = cleanFirestoreData({
      merchantConfirmedDelivery: true,
      merchantConfirmedAt: new Date(),
      // keep orderStatus as 'shipped' until customer confirms receipt
      orderStatus: "shipped",
      updatedAt: new Date(),
    });
  ...
  // Customer confirms they received the order — finalizes delivery
  async confirmDeliveryByCustomer(orderId: string): Promise<void> {
    const cleanedData = cleanFirestoreData({
  ...
  // Update shipping address with governorate field (Yemeni support)
  async updateShippingAddressWithGovernorate(
    orderId: string,
    ...
    try {
      ...
      console.error("Error updating shipping address with governorate:", error);
      throw error;
    }
  },
};
...
export const customerService = {
  ...
  async update(customerId: string, data: Partial<Customer>): Promise<void> {
    ...
    await updateDoc(doc(db, "customers", customerId), cleanedData);
  },

  async createOrUpdateFromUser(userData: any): Promise<string> {
    try {
      const { uid, email, firstName, lastName, phone } = userData;

      const existingCustomer = await this.getByUid(uid);
      ...
      console.error("Error creating or updating customer from user:", error);
      throw error;
    }
  },
  ...
  async getByStore(storeId: string): Promise<Customer[]> {
    try {
      const storeOrders = await orderService.getByStore(storeId);
      ...
      const customerIds = [
        ...new Set(storeOrders.map((order) => order.customerId)),
      ];
      ...
      for (const customerId of customerIds) {
        try {
          ...
          if (customer) {
            ...
            } as Customer & { totalOrders: number; totalSpent: number };

            customers.push(customerWithStats);
          }
        } catch (error) {
          console.error(`❌ خطأ في جلب العميل ${customerId}:`, error);
        }
      }
  ...
  async updateShippingAddress(
    ...
    try {
      ...
      console.error("❌ خطأ في تحديث عنوان الشحن:", error);
      throw error;
    }
  },
};
...
export const cartService = {
  async getCustomerCart(
    ...
    try {
      ...
      if (!querySnapshot.empty) {
        const doc = querySnapshot.docs[0];
        return { id: doc.id, ...doc.data() } as CustomerCart;
      }

      return await this.createCart(customerId, storeId);
    } catch (error) {
  ...
  async createCart(customerId: string, storeId: string): Promise<CustomerCart> {
    const cartData = {
      customerId,
      storeId,
      items: [],
    ...
    const docRef = await addDoc(collection(db, "customerCarts"), cartData);
  ...
  async addToCart(
    ...
    const cartDoc = await getDoc(doc(db, "customerCarts", cartId));
    if (!cartDoc.exists()) return;

    const cart = cartDoc.data() as CustomerCart;
    ...
    await updateDoc(doc(db, "customerCarts", cartId), {
      items: newItems,
      updatedAt: new Date(),
    });
  },
    ...
    try {
      await updateDoc(doc(db, "customerCarts", cartId), {
        items: items.map((item) => ({
          ...
          addedAt: this.convertToFirestoreDate(item.addedAt),
        })),
        updatedAt: new Date(),
      });
    } catch (error) {
  ...
  convertToFirestoreDate(date: any): Date {
...
export const favoritesService = {
  async getCustomerFavorites(
    customerId: string,
    storeId: string,
  ): Promise<CustomerFavorites | null> {
    try {
      const q = query(
      ...
      if (!querySnapshot.empty) {
        const doc = querySnapshot.docs[0];
        return { id: doc.id, ...doc.data() } as CustomerFavorites;
      }

      return await this.createFavorites(customerId, storeId);
    } catch (error) {
  ...
  async createFavorites(
    ...
    return { id: docRef.id, ...favoritesData };
  },

  async addToFavorites(favoritesId: string, productId: string): Promise<void> {
    const favoritesDoc = await getDoc(
      doc(db, "customerFavorites", favoritesId),
    );
    if (!favoritesDoc.exists()) return;

    const favorites = favoritesDoc.data() as CustomerFavorites;
    ...
    await updateDoc(doc(db, "customerFavorites", favoritesId), {
      items: newItems,
      updatedAt: new Date(),
    });
  },
  ...
  async removeFromFavorites(
    ...
    const favorites = favoritesDoc.data() as CustomerFavorites;
    const newItems = favorites.items.filter(
      (item) => item.productId !== productId,
    );
  ...
  async isProductInFavorites(
    ...
    if (!favoritesDoc.exists()) return false;
  ...
  async updateFavorites(
    ...
    try {
      const items: FavoriteItem[] = productIds.map((productId) => ({
        productId,
        addedAt: new Date(),
      }));
  ...
  async createOrUpdateFavorites(
    customerId: string,
    storeId: string,
    productIds: string[],
  ): Promise<CustomerFavorites> {
    try {
      const existingFavorites = await this.getCustomerFavorites(
        customerId,
        storeId,
      );

      if (existingFavorites) {
        await this.updateFavorites(existingFavorites.id, productIds);
        return {
          ...existingFavorites,
        ...
        return await this.createFavoritesWithItems(
          customerId,
          storeId,
          productIds,
        );
      ...
      console.error("Error creating or updating favorites:", error);
      throw error;
    }
  },

  async createFavoritesWithItems(
    customerId: string,
    storeId: string,
    productIds: string[],
  ): Promise<CustomerFavorites> {
...
export const updateOrderShippingAddressWithGovernorate =
  orderService.updateShippingAddressWithGovernorate;
export const updateCustomerShippingAddress =
  customerService.updateShippingAddress;
