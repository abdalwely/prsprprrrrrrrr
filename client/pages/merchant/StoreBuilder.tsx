...
import {
  ...
  validateStoreCustomization,
} from "@/lib/storeDefaults";
import type { StoreCustomization } from "@/lib/enhanced-templates";
import { convertTemplateToCustomization } from "@/lib/template-converter";

export default function StoreBuilder() {
  const [language, setLanguage] = useState<"ar" | "en">("ar");
  const [currentStep, setCurrentStep] = useState<
    "template" | "customize" | "settings" | "preview"
  >("template");
  ...
  >("desktop");
  const [currentStore, setCurrentStore] = useState<any>(null); // ✅ إضافة state للمتجر الحالي
  ...
  // ✅ جديد: تحميل المتجر الحالي
  useEffect(() => {
    const loadCurrentStore = async () => {
      if (userData?.uid) {
        try {
          console.log("🔍 Loading current store for user:", userData.uid);
          ...
          if (userStores.length > 0) {
            ...
            // ✅ تحميل القالب الحالي إذا كان موجوداً
            if (store.template) {
              const template = getTemplateById(store.template);
              if (template) {
                setSelectedTemplate(template);
                setCustomizedTemplate(template);
                console.log("✅ Loaded existing template:", store.template);
              }
            }

            console.log("✅ Current store loaded:", store.name);
          } else {
  ...
  const handleColorChange = (colorKey: string, value: string) => {
    if (customizedTemplate) {
      const updatedTemplate = updateTemplateColors(customizedTemplate, {
        [colorKey]: value,
      });
  ...
  const handleSaveStore = async () => {
    ...
    try {
      ...
      // ✅ 1. حذف المتاجر القديمة أولاً
      const userStores = await storeService.getByOwner(userData.uid);
      console.log("📊 Found user stores:", userStores.length);

      for (const oldStore of userStores) {
        if (!currentStore || oldStore.id !== currentStore.id) {
          await storeService.delete(oldStore.id);
          console.log("🗑️ Deleted old store:", oldStore.name);
        }
      }

      // ✅ 2. تحويل القالب البسيط إلى StoreCustomization الشامل
      const fullCustomization =
        convertTemplateToCustomization(customizedTemplate);

      // ✅ 3. إذا كان هناك متجر حالي، قم بتحديثه
      if (currentStore) {
        const updates = {
          name:
            storeData.name ||
            userData.businessName ||
            `${userData.firstName}'s Store`,
        ...
        await storeService.update(currentStore.id, updates);
        console.log("✅ Store updated successfully:", currentStore.id);
      } else {
        // ✅ 4. إذا لم يكن هناك متجر، أنشئ متجر جديد
        const storePayload = {
        ...
        console.log("✅ New store created successfully:", storeId);
      }

      toast({
        title: currentText.success,
        description:
          "تم حفظ المتجر بنجاح! يمكنك الآن إدارة متجرك من لوحة التحكم",
      });

      // ✅ 5. التوجيه إلى Dashboard بعد الحفظ
      ...
      toast({
        ...
        description: "حدث خطأ أثناء حفظ المتجر. يرجى المحاولة مرة أخرى",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const renderTemplateGrid = () => (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {storeTemplates.map((template) => (
        <Card
          key={template.id}
          className="cursor-pointer hover:shadow-lg transition-shadow card-shadow"
        >
        ...
        <Tabs defaultValue="colors">
              ...
              <div>
                ...
                <div className="flex space-x-2">
                  <Input
                    type="color"
                    value={
                      customizedTemplate?.customization.colors.primary ||
                      "#FF6B35"
                    }
                    onChange={(e) =>
              ...
              <div>
                ...
                <div className="flex space-x-2">
                  <Input
                    type="color"
                    value={
                      customizedTemplate?.customization.colors.secondary ||
                      "#4A90E2"
                    }
                    onChange={(e) =>
              ...
              <div>
                ...
                <div className="flex space-x-2">
                  <Input
                    type="color"
                    value={
                      customizedTemplate?.customization.colors.background ||
                      "#FFFFFF"
                    }
                    onChange={(e) =>
              ...
              <div>
                ...
                <div className="flex space-x-2">
                  <Input
                    type="color"
                    value={
                      customizedTemplate?.customization.colors.text || "#333333"
                    }
          ...
          <TabsContent value="typography" className="space-y-4">
            <div>
              ...
              <Select
                value={
                  customizedTemplate?.customization.fonts.primary || "Cairo"
                }
                onValueChange={(value) =>
            ...
            <div>
              ...
              <Select
                value={
                  customizedTemplate?.customization.fonts.secondary || "Inter"
                }
                onValueChange={(value) =>
                  setCustomizedTemplate((prev) =>
                    prev
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="flex items-center space-x-2">
                <input
                  ...
                  id="showCategories"
                  checked={
                    customizedTemplate?.customization.layout.showCategories ||
                    true
                  }
                  onChange={(e) =>
                    handleLayoutChange("showCategories", e.target.checked)
                  }
                />
                <Label htmlFor="showCategories">
                  {currentText.showCategories}
                </Label>
              </div>
              ...
              <div className="flex items-center space-x-2">
                <input
                  ...
                  id="showSearch"
                  checked={
                    customizedTemplate?.customization.layout.showSearch || true
                  }
                  onChange={(e) =>
                    handleLayoutChange("showSearch", e.target.checked)
                  }
                />
              ...
              <div className="flex items-center space-x-2">
                <input
                  ...
                  id="showWishlist"
                  checked={
                    customizedTemplate?.customization.layout.showWishlist ||
                    false
                  }
                  onChange={(e) =>
                    handleLayoutChange("showWishlist", e.target.checked)
                  }
                />
              ...
              <div className="flex items-center space-x-2">
                <input
                  ...
                  id="showCart"
                  checked={
                    customizedTemplate?.customization.layout.showCart || true
                  }
                  onChange={(e) =>
                    handleLayoutChange("showCart", e.target.checked)
                  }
                />
              ...
              <div className="flex items-center space-x-2">
                <input
                  ...
                  id="heroSection"
                  checked={
                    customizedTemplate?.customization.layout.heroSection || true
                  }
                  onChange={(e) =>
                    handleLayoutChange("heroSection", e.target.checked)
                  }
                />
              ...
              <div className="flex items-center space-x-2">
                <input
                  ...
                  id="featuredProducts"
                  checked={
                    customizedTemplate?.customization.layout.featuredProducts ||
                    true
                  }
                  onChange={(e) =>
                    handleLayoutChange("featuredProducts", e.target.checked)
                  }
                />
                <Label htmlFor="featuredProducts">
                  {currentText.featuredProducts}
                </Label>
              </div>
            </div>
          </TabsContent>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="flex items-center space-x-2">
                <input
                  ...
                  id="testimonials"
                  checked={
                    customizedTemplate?.customization.layout.testimonials ||
                    false
                  }
                  onChange={(e) =>
                    handleLayoutChange("testimonials", e.target.checked)
                  }
                />
              ...
              <div className="flex items-center space-x-2">
                <input
                  ...
                  id="newsletter"
                  checked={
                    customizedTemplate?.customization.layout.newsletter || false
                  }
                  onChange={(e) =>
                    handleLayoutChange("newsletter", e.target.checked)
                  }
                />
              ...
              <div className="p-4">
                <div className="grid grid-cols-2 gap-2">
                  <div
                    ...
                    style={{
                      backgroundColor:
                        customizedTemplate.customization.colors.accent ||
                        "#3b82f6",
                    }}
                  >
                    <div className="p-2">
    ...
    <div className="max-w-md mx-auto space-y-6">
      ...
      <div>
        ...
        {currentStore?.subdomain && (
          <p className="text-sm text-gray-500 mt-1">
            النطاق الحالي: {currentStore.subdomain}.platform.com
          </p>
        )}
      </div>
      ...
      <div>
        ...
        <Input
          ...
          onChange={(e) =>
            handleStoreDataChange("customDomain", e.target.value)
          }
          placeholder="www.mystore.com (اختياري)"
        />
      </div>

      {/* ✅ إظهار معلومات المتجر الحالي */}
      {currentStore && (
        <div className="p-4 bg-blue-50 rounded-lg">
          <h4 className="font-medium text-blue-800 mb-2">المتجر الحالي</h4>
          <p className="text-sm text-blue-700">الاسم: {currentStore.name}</p>
          <p className="text-sm text-blue-700">الحالة: {currentStore.status}</p>
          <p className="text-sm text-blue-700">سيتم تحديث هذا المتجر</p>
        </div>
      )}
    </div>
  );
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-center space-x-4">
            {["template", "customize", "settings", "preview"].map(
              (step, index) => (
                  ...
                  <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center ${
                      currentStep === step
                        ...
                        : ["template", "customize", "settings"].indexOf(
                              currentStep,
                            ) > index
            ...
            {currentStep === "preview" && (
              <div className="text-center">
                ...
                <p className="text-gray-600 mb-6">
                  يمكنك الآن نشر متجرك وبدء إضافة المنتجات
                </p>
                {currentStore && (
                  <div className="bg-green-50 p-4 rounded-lg mb-4">
                    <p className="text-green-800 font-medium">
                      سيتم تحديث المتجر الحالي: {currentStore.name}
                    </p>
                    <p className="text-green-600 text-sm">
                      الحالة ستتغير من {currentStore.status} إلى active
                    </p>
                  </div>
                )}
              </div>
        ...
        <div className="flex justify-between mt-8">
          ...
          <Button
            ...
            disabled={
              (currentStep === "template" && !selectedTemplate) || loading
            }
