...
// إنشاء نوع Store مع خصائص اختيارية للتحقق الآمن
...
export default function AdminDashboard() {
  ...
  // دالة مساعدة لتحويل التاريخ إلى تنسيق آمن
  const safeDateToString = (date: any): string => {
    if (isValidDate(date)) {
      return new Date(date).toISOString();
    }
    return new Date().toISOString(); // تاريخ افتراضي إذا كان التاريخ غير صالح
  };
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      // Calculate stats
      const activeStores = cleanedStores.filter(
        (store) => store.status === "active",
      ).length;
      const pendingStores = cleanedStores.filter(
        (store) => store.status === "pending",
      ).length;
      const suspendedStores = cleanedStores.filter(
        (store) => store.status === "suspended",
      ).length;

      // Mock additional stats
      setStats({
        totalStores: cleanedStores.length,
        activeStores,
        pendingStores,
        suspendedStores,
        totalMerchants: cleanedStores.length,
        totalCustomers: 2500,
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">
            {currentText.welcome}, {userData?.firstName || "المشرف"}!
          </h1>
          ...
          <Card className="card-shadow">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-gray-600">
                    {currentText.platformCommission}
                  </p>
                  <p className="text-3xl font-bold text-gray-900">
        ...
        <Card className="card-shadow mb-8">
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {/* زر مراجعة الطلبات الجديد */}
              <Link to="/admin/store-approvals">
                <Button className="w-full justify-start btn-gradient relative">
                  <FileCheck className="h-4 w-4 mr-2 rtl:ml-2 rtl:mr-0" />
                  {currentText.reviewApplications}
                  {stats.pendingStores > 0 && (
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription>
                  إدارة ومراقبة جميع المتاجر على المنصة
                </CardDescription>
              </div>
              {stats.pendingStores > 0 && (
                <Badge className="bg-yellow-100 text-yellow-800">
            ...
            {filteredStores.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-gray-600">
                  لا توجد متاجر مطابقة للمعايير المحددة
                </p>
              </div>
            ) : (
                        ...
                        <td className="py-3 px-2">
                          <div className="text-sm">
                            <div className="font-medium">
                              {store.contact?.email || currentText.noEmail}
                            </div>
                            <div className="text-gray-600">
                              {store.contact?.phone || currentText.noPhone}
                            </div>
                          </div>
                        </td>
                        <td className="py-3 px-2">
