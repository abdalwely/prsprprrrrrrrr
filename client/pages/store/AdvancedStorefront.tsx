...
//     storeSyncManager.addEventListener(
//       "products-updated",
...
//     return () => {
//       clearTimeout(reloadTimeout);
//       storeSyncManager.removeEventListener(
//         "stores-updated",
//         handleStoresUpdated,
//       );
//       storeSyncManager.removeEventListener(
...
//     try {
//       setLoading(true);
//       console.log("🔍 Starting store search for subdomain:", subdomain);
...
//       // الحل الأول: البحث في Firebase باستخدام الـ subdomain
//       try {
...
//         console.log("🔥 Searching in Firebase for subdomain:", subdomain);

//         // إنشاء query للبحث بالـ subdomain
...
//           console.log("✅ STORE FOUND IN FIREBASE:", {
//             id: storeId,
//             name: storeData.name,
...
//           console.log("✅ Store data loaded successfully from Firebase:", {
...
//         } else {
//           console.log(
//             "❌ No store found in Firebase for subdomain:",
//             subdomain,
//           );
//         }
//       } catch (firebaseError) {
...
//       // الحل الثاني: البحث في البيانات المحلية (fallback)
//       console.log("🔄 Falling back to local data search...");
//       let stores = storeSyncManager.getStoresWithFallback();
//       console.log("📦 Available stores from sync:", stores.length);

//       if (stores.length === 0) {
//         console.log("⏳ No stores found in sync, trying localStorage...");
...
//               }));
//               console.log("📦 Loaded stores from localStorage:", stores.length);
//             }
//           }
//         } catch (localStorageError) {
//           console.error(
//             "Error reading stores from localStorage:",
//             localStorageError,
//           );
//         }
...
//           stores = await waitForStoreData(subdomain, 3000);
//         }
//       }

//       // البحث عن المتجر بالـ subdomain في البيانات المحلية
...
//             (subdomain && subdomain.includes(s.subdomain)),
//         );
//         console.log("🔍 Trying partial subdomain match");
//       }

//       if (foundStore) {
//         console.log("✅ STORE FOUND IN LOCAL DATA:", {
//           id: foundStore.id,
...
//         console.log("✅ Store data loaded successfully from local data:", {
//           products: storeProducts.length,
//           categories: storeCategories.length,
//         });
...
//       // إذا لم يتم العثور على المتجر نهائياً
...
//       setLoading(false);
//     } catch (error) {
//       console.error("❌ Error in loadStoreData:", error);
//       setLoading(false);
//     }
//   };
...
//         "--store-header-bg":
//           store.customization.colors.headerBackground ||
//           store.customization.colors.background,
//         "--store-footer-bg":
//           store.customization.colors.footerBackground ||
//           store.customization.colors.secondary,
//         "--store-card-bg":
//           store.customization.colors.cardBackground || "#ffffff",
//         "--store-border": store.customization.colors.borderColor || "#e5e7eb",
//       } as React.CSSProperties)
//     : {};

//   const containerClass =
//     store?.customization.layout.containerWidth === "full"
//       ? "max-w-full"
//       : store?.customization.layout.containerWidth === "wide"
//         ? "max-w-8xl"
//         : store?.customization.layout.containerWidth === "narrow"
//           ? "max-w-4xl"
//           : "max-w-7xl";

//   const spacingClass =
//     store?.customization.layout.spacing === "tight"
//       ? "space-y-4"
//       : store?.customization.layout.spacing === "relaxed"
//         ? "space-y-12"
//         : store?.customization.layout.spacing === "loose"
//           ? "space-y-16"
//           : "space-y-8";
...
//             <p className="text-gray-600 mb-6">
//               رقم الطلب:{" "}
//               <strong>{completedOrderId.slice(-8).toUpperCase()}</strong>
//             </p>
...
//                 style={{
//                   backgroundColor:
//                     store?.customization.colors.primary || "#2563eb",
//                   color: "white",
//                 }}
//               >
//                 تتبع الطلب
...
//               {store.settings.shipping.freeShippingThreshold} ر.س
//             </span>
//           </div>
//           <div className="flex items-center gap-2">
...
//               style={{
//                 backgroundColor:
//                   store?.customization.colors.background || "#ffffff",
...
//             اكتشف مجموعتنا المتنوعة من المنتجات المصنفة خصيصاً لتلبية احتياجاتك
//           </p>
//         </div>
...
//                   style={{
//                     backgroundColor: `${store?.customization.colors.primary}20`,
//                   }}
//                 >
//                   <Package
//                     className="h-8 w-8"
...
//         <p className="text-gray-600 max-w-2xl mx-auto">
//           اختيارنا الخاص من أفضل المنتجات التي تلقى إعجاب عملائنا
//         </p>
//       </div>

//       {products.filter((p) => p.featured && p.status === "active").length >
//       0 ? (
...
//             سيتم عرض المنتجات المميزة هنا قريباً
//           </p>
...
//               style={{
//                 color: store?.customization.colors.primary || "#16a34a",
//               }}
//             >
//               {product.price} ر.س
//             </span>
...
//     return total >= store.settings.shipping.freeShippingThreshold
//       ? 0
//       : store.settings.shipping.defaultCost;
//   };
...
//                             style={{
//                               color:
//                                 store?.customization.colors.primary ||
//                                 "#16a34a",
//                             }}
//                           >
