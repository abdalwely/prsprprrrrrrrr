...
export default function DiagnosticsPage() {
  ...
  const loadData = async () => {
    try {
      ...
      setStores(allStores);
      setApplications(await allApplications);
      setProducts(allProducts);

      const logs = [
        `المتاجر المحفوظة: ${allStores.length}`,
        `طلبات الموافقة: ${(await allApplications).length}`,
  ...
  const fixMerchantNames = async () => {
    try {
      ...
      allStores.forEach((store) => {
        ...
        const merchantApp = applications.find(
          (app: any) => app.merchantId === store.ownerId,
        );
        if (merchantApp) {
          const newName = `متجر ${merchantApp.merchantData.firstName}`;
          console.log(
            `Found merchant data: ${merchantApp.merchantData.firstName} ${merchantApp.merchantData.lastName}`,
          );

          if (store.name !== newName) {
            console.log(
              `✏️ Updating store name from "${store.name}" to "${newName}"`,
            );
            // تحديث اسم المتجر
          ...
          console.log(
            `❌ No merchant application found for owner: ${store.ownerId}`,
          );
        }
      });
      ...
      if (userData && userData.userType === "merchant") {
        ...
        const merchantApp = applications.find(
          (app: any) =>
            app.merchantData.email.toLowerCase() ===
            userData.email?.toLowerCase(),
        );

        if (merchantApp) {
          console.log(
            `Found current user's merchant data: ${merchantApp.merchantData.firstName}`,
          );
          ...
          console.log(
            "✅ تم إصلاح بيانات المستخدم:",
            merchantApp.merchantData.firstName,
          );
        } else {
      ...
      <div className="max-w-6xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-2">
            أدوات لاختبار وتشخيص مشاكل النظام
          </p>
        </div>
              ...
              <div className="space-y-2 max-h-60 overflow-y-auto">
                {stores.map((store) => (
                  ...
                  >
                    ...
                    <div>
                      <strong>الرابط:</strong> {store.subdomain}
                    </div>
                    <div>
                      <strong>المالك:</strong> {store.ownerId}
                    </div>
                    <div>
                      <strong>الألوان:</strong>{" "}
                      {store.customization?.colors?.primary || "غير محدد"}
                    </div>
                    <Button
                      size="sm"
                      className="mt-2"
              ...
              <div className="space-y-2 max-h-60 overflow-y-auto">
                {applications.map((app) => (
                  <div key={app.id} className="p-2 bg-gray-100 rounded text-sm">
                    ...
                    <div>
                      <strong>التاجر:</strong> {app.merchantData.firstName}
                    </div>
                    <div>
                      <strong>الحالة:</strong> {app.status}
                    </div>
                    {app.status === "pending" && (
                      <Button
                        size="sm"
                        className="mt-2"
                        onClick={async () => {
                          const success = await approveStoreApplication(
                            app.id,
                            "admin_test",
                          );
                          if (success) {
